...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  return (
    <div className={`sm-scope z-50 ${isFixed ? 'fixed top-0 left-0 w-screen h-screen overflow-hidden pointer-events-none' : 'w-full h-full pointer-events-none'}`}>
      <div
      ...
      >
        ...
        <header className="staggered-menu-header absolute top-0 left-0 w-full flex items-center justify-between p-[2em] bg-transparent pointer-events-none z-20" aria-label="Main navigation header">
          <div className="sm-logo flex items-center select-none pointer-events-auto" aria-label="Logo">
            <span className="text-xl font-bold text-white tracking-tight">{logoText}</span>
          </div>
...
@media (max-width: 1024px) { .sm-scope .staggered-menu-panel { width: 100%; left: 0; right: 0; } .sm-scope .sm-prelayers { width: 100%; } .sm-scope .staggered-menu-wrapper[data-open] .sm-logo { filter: invert(100%); } }
@media (max-width: 640px) { .sm-scope .staggered-menu-panel { width: 100%; left: 0; right: 0; } .sm-scope .sm-prelayers { width: 100%; } .sm-scope .staggered-menu-wrapper[data-open] .sm-logo { filter: invert(100%); } .sm-scope .sm-panel-item { font-size: 2.5rem; } }
      `}</style>
    </div>
  );
};
