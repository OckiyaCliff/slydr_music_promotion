...
class LightsSticks {
  ...
  init() {
    ...
    if (Array.isArray(options.colors.sticks)) {
      colorArray = (options.colors.sticks as any as number[]).map((c: number) => new THREE.Color(c));
    } else {
...
class Road {
  ...
  createPlane(side: number, width: number, isRoad: boolean) {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      side: THREE.DoubleSide,
      uniforms: Object.assign(
        uniforms, this.webgl.fogUniforms,
        (typeof options.distortion === 'object' ? options.distortion.uniforms : {}) || {}
      )
    });
...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    this.onTouchStart = (ev: TouchEvent) => {
      if (this.options.onSpeedUp) this.options.onSpeedUp(ev);
      this.fovTarget = this.options.fovSpeedUp;
      this.speedUpTarget = this.options.speedUp;
    };
    this.onTouchEnd = (ev: TouchEvent) => {
      if (this.options.onSlowDown) this.options.onSlowDown(ev);
      this.fovTarget = this.options.fov;
      this.speedUpTarget = 0;
    };
    this.onContextMenu = (ev: MouseEvent) => { ev.preventDefault(); };
